...
class AlertGenerationModule:
    """
    Handles the creation and negotiation of data alerts based on SQL queries.
    """
    ...
    async def process_alert_request(self, base_sql: str, user_message: str, history: list) -> Tuple[str, str, Optional[str], Optional[Dict[str, Any]]]:
        ...
        prompt = f"""
You are an expert SQL engineer. Your task is to generate a specific SQL query for a DATA ALERT system.
...
Example:
Base: SELECT count(*) as fail_count FROM logs WHERE status='error'
User: "Alert if this goes above 50"
Result:
{{
  "status": "created",
  "response_message": "Done! I'll alert you if the error count exceeds 50.",
  "alert_sql": "SELECT count(*) as fail_count FROM logs WHERE status='error' HAVING count(*) > 50",
  "alert_config": {{ ... }}
}}
"""
        try:
